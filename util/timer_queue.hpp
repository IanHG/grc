...
class timer_queue 
{
   public:
      ...
      // timer
      template<class D>
         uint64_t add(const D& duration, std::function<void(bool)> handler) {
            WorkItem item;
            item.end = Clock::now() + std::chrono::milliseconds(duration);
   ...
   private:
      ...
      void checkWork() {
         ...
         while (m_items.size() && m_items.top().end <= Clock::now()) {
            ...
            lk.unlock();
            if (item.handler)
               m_pool.submit([handler = item.handler, id = item.id](){ handler(id == 0); });
            lk.lock();
         }
      }
